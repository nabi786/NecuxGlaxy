...
const storage = multer.diskStorage({
 ...
 const upload= multer({
     ...
     fileFilter: fileFilter}).fields([{name:'avatar', maxCount:1}, {name: 'background', maxCount:1}
]);

router.route('/create').post(upload,auth,CollectionController.createCollection);
router.route('/update').post(upload,auth,CollectionController.updateCollection);
...
router.route('/remove/nft').post(auth,CollectionController.removeNFTFromCollection);
